...
```{r, chunk5.2l, include = inc.lattice}
# plot it all
...
```{r, chunk5.2g, include = inc.gg}
# plot it all
ggplot(WaffleDivorce) +
  geom_point(aes(y = Divorce,  x = MedianAgeMarriage.s, state = Loc), 
             col = rangi2) +
...
```{r, chunk5.8l, include = inc.lattice}
...
```{r, chunk5.8g, include = inc.gg, include = inc.gg}
...
```{r, chunk5.9l, include = inc.lattice}
...
```{r, 5.9g, include = inc.gg, include = inc.gg}
...
```
```{r, chunk5.10l, include = inc.lattice}
...
```{r, 5.10g, include = inc.gg, include = inc.gg}
...
```{r, chunk5.12l, include = inc.lattice}
xyplot(
  mu.mean ~ Divorce,
  data = m5.3.pred3,
  col = rangi2,
  ...
  panel = function(x, y, ..., y0, y1) {
    ...
    with(m5.3.pred3, panel.segments(x0 = x, x1 = x, y0 = divorce.PIlo, y1 = divorce.PIhi))
  }
)
```


```{r, chunk5.12g, include = inc.gg, include = inc.gg}
ggplot(data = m5.3.pred3, 
       aes(x = Divorce, y = mu.mean, ymin = divorce.PIlo, ymax = divorce.PIhi, text = Loc)) +
...
```{r, chunk5.23l, include = inc.lattice}
...
```{r, chunk5.32l, include = inc.lattice}
xyplot(bl ~ br, data = m5.8.post, col = rangi2, alpha = 0.1, pch = 16)
```

```{r, chunk5.32g, include = inc.gg}
ggplot(m5.8.post) +
...
```{r, chunk5.33l, include = inc.lattice}
densityplot(~(bl + br), data = m5.8.post)
```
```{r, chunk5.33gg, include = inc.gg}
ggplot(m5.8.post) +
  geom_area(aes(x = bl + br), alpha = 0.3, stat = "density")
```
...
```{r, chunk5.38l, include = inc.lattice}
...
```{r, chunk5.40l, include = inc.lattice} 
xyplot(
  sd ~ r,
  data = SimData5.40,
